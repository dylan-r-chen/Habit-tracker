...
function getWeekCompletionRate(habits, completions, weekStart) {
  ...
  const goalHabits = activeHabits.filter(h => h.goal);
  if (!goalHabits.length) {
    let total = 0, done = 0;
    activeHabits.forEach(h => {
      if (h.freq === "daily") {
        total += 7;
        done += days.filter(d => completions[h.id]?.[dateKey(d)]).length;
      } else {
        total++;
        if (days.some(d => completions[h.id]?.[dateKey(d)])) done++;
      }
    });
    return total === 0 ? 0 : Math.round((done / total) * 100);
  }
  const pcts = goalHabits.map(h => {
    const count = days.filter(d => completions[h.id]?.[dateKey(d)]).length;
    if (h.goal.period === "week") return Math.min(count / h.goal.target, 2) * 100;
    if (h.goal.period === "month") return Math.min(count / (h.goal.target / 4.33), 2) * 100;
    return Math.min(count / (h.goal.target / 52), 2) * 100;
  });
